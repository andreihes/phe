...
int calc_OT4(const int* hands, const int* scenes, const int deck[52], const int ftrsz[5], char* json) {
        ...
        for(int d3 = d2 + 1; d3 < ftrsz[3]; ++d3) {
          for(int d4 = d3 + 1; d4 < ftrsz[4]; ++d4) {
            ...
            int h3r = eval_OT(deck[d0], deck[d1], deck[d2], deck[d3], deck[d4], h3[0], h3[1], h3[2], h3[3], h3[4]);
                                                                        // 4 choose 1 = 4
                 if(h0r <  h1r && h0r <  h2r && h0r <  h3r) ++wins[0];  // 0
            else if(h1r <  h0r && h1r <  h2r && h1r <  h3r) ++wins[1];  // 1
            else if(h2r <  h0r && h2r <  h1r && h2r <  h3r) ++wins[2];  // 2
            else if(h3r <  h0r && h3r <  h1r && h3r <  h2r) ++wins[3];  // 3
                                                                        // 4 choose 2 = 6
            else if(h0r == h1r && h0r <  h2r && h0r  < h3r) ++ties[0];  // 0 1
            else if(h0r == h2r && h0r <  h1r && h0r  < h3r) ++ties[1];  // 0 2
            else if(h0r == h3r && h0r <  h1r && h0r  < h2r) ++ties[2];  // 0 3
            else if(h1r == h2r && h1r <  h0r && h1r  < h3r) ++ties[3];  // 1 2
            else if(h1r == h3r && h1r <  h0r && h1r  < h2r) ++ties[4];  // 1 3
            else if(h2r == h3r && h2r <  h0r && h2r  < h1r) ++ties[5];  // 2 3
                                                                        // 4 choose 3 = 4
            else if(h0r == h1r && h0r == h2r && h0r  < h3r) ++ties[6];  // 0 1 2
            else if(h0r == h1r && h0r == h3r && h0r  < h2r) ++ties[7];  // 0 1 3
            else if(h0r == h2r && h0r == h3r && h0r  < h1r) ++ties[8];  // 0 2 3
            else if(h1r == h2r && h1r == h3r && h1r  < h0r) ++ties[9];  // 1 2 3
                                                                        // 4 choose 4 = 1
            else if(h0r == h1r && h0r == h2r && h0r == h3r) ++ties[10]; // 0 1 2 3
  ...
  (
    ...
    scenes[1], scenes[2], ties[3],
    scenes[1], scenes[3], ties[4],
    scenes[2], scenes[3], ties[5],
    scenes[0], scenes[1], scenes[2], ties[6],
    scenes[0], scenes[1], scenes[3], ties[7],
    scenes[0], scenes[2], scenes[3], ties[8],
    scenes[1], scenes[2], scenes[3], ties[9],
    scenes[0], scenes[1], scenes[2], scenes[3], ties[10]
  );
  repl(json);
  return 0;
}
